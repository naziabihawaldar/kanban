...
use App\Exceptions\CardImportException;
...
class CardsImport implements ToModel, WithHeadingRow
{
     ...
     */
    public $boardId, $imprt_file, $log_file;

    public function __construct($board_id, $import_file, $log_file)
    {
    ...
    public function model(array $row)
    {
        try 
        {
            ...
            if ($board) {
                ...
                if ($column) {
                    ...
                    $newRecord->save();
                    if(isset($row['assigned_by_email']) && $row['assigned_by_email'] != '')
                    {
        ...
        } catch (CardImportException $e) {
            //throw $th;
            array_push($row, $e->getMessage());
            array_push($row, 'Not inserted');
            fputcsv($myfile, $row);
        }
